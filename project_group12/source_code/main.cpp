...
string mapFilePath = "../sourses/map/testEventMap5.txt";
...
bool detectEnding(Player& player) {
    if (finals) {
        ...
        // FIX CONDITIONS
        if (player.getEmo() >= 12) {
            ifstream inFile("../sourses/ending/ending3.txt");
            animateStringForEnding(inFile);
            inFile.close();
        }
        else if (player.getAcademic() + player.getSocial() >= 30) {
            ifstream inFile("../sourses/ending/ending2.txt");
            animateStringForEnding(inFile);
            inFile.close();
        }
        else if (player.getAcademic() >= 10) {
            ifstream inFile("../sourses/ending/ending1.txt");
            animateStringForEnding(inFile);
            inFile.close();
        }
        else {
